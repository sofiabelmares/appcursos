...
export class AsignarAlumnosComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<Alumno>; //para paginacion
  @ViewChild(MatPaginator, {static: true}) paginator: MatPaginator;
  pageSizeOptions: number[] = [3, 5, 10, 20, 50]

  seleccion: SelectionModel<Alumno> = new SelectionModel<Alumno>(true, []); //para seleccionar alumnos

  constructor(private route: ActivatedRoute,
    private cursoService: CursoService,
    private alumnoService: AlumnoService) { }

  ngOnInit() { //implementar observador
  ...
  private iniciarPaginador(): void{
    this.dataSource = new MatTableDataSource<Alumno>(this.alumnos);//Crear la instancia del dataSource
      ...
      .subscribe(c => {
        ...
        e => {
          if (e.status === 500) {
            ...
            if (mensaje.indexOf('ConstraintViolationException') > -1) { //si se encontró
              Swal.fire(
                'Error',
                'El alumno ya está asignado a otro curso',
                'error'
              );
            }
          }
  ...
  eliminarAlumno(alumno: Alumno): void {
    Swal.fire({
      title: 'Confirmar',
      text: `¿Seguro que desea eliminar a ${alumno.nombre} ${alumno.apellido} del curso?`,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.cursoService.eliminarAlumno(this.curso, alumno)
          .subscribe(curso => {
            ...
            Swal.fire(
              'Eliminado',
              `Se ha eliminado a ${alumno.nombre} ${alumno.apellido} del curso ${curso.nombre}`,
              'success'
            );
          });
      }
