...
public class RespuestaController {
	...
	@Autowired //se ocupa para guardar las respuestas
	private RespuestaService service;
	
	//Se guarda un conjunto de respuestas
	@PostMapping
	public ResponseEntity<?> crear(@RequestBody Iterable<Respuesta> respuestas){
		respuestas = ((List<Respuesta>) respuestas).stream().map(r -> {
			r.setAlumnoId(r.getAlumno().getId());//tomamos el alumno transient del json
			r.setPreguntaId(r.getPregunta().getId()); //establecer id para pregunta
			return r; //se retorna la respuesta que se esta emitinendo por cada respuesta pero modficada
